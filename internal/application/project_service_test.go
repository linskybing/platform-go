...
func TestProjectServiceCRUD(t *testing.T) {
	...
	t.Run("CreateProject success", func(t *testing.T) {
		...
		mockGroup.EXPECT().GetGroupByID("1").Return(group.Group{GID: "1", GroupName: "test-group"}, nil)
		...
		mockProject.EXPECT().GetProjectByID("1").Return(project.Project{PID: "1", ProjectName: "proj1", GID: "1"}, nil).AnyTimes()
		...
		if proj.PID != "1" {
			t.Fatalf("expected PID 1, got %s", proj.PID)
		}
	})
	...
	t.Run("UpdateProject success", func(t *testing.T) {
		...
		mockProject.EXPECT().GetProjectByID("1").Return(oldProject, nil).AnyTimes()
	...
	t.Run("UpdateProject not found", func(t *testing.T) {
		mockProject.EXPECT().GetProjectByID("99").Return(project.Project{}, errors.New("not found")).AnyTimes()
		newName := "test"
		input := project.UpdateProjectDTO{ProjectName: &newName}
	...
	t.Run("DeleteProject success", func(t *testing.T) {
		...
		mockProject.EXPECT().GetProjectByID("1").Return(proj, nil).AnyTimes()
		mockProject.EXPECT().DeleteProject("1").Return(nil)
...
func TestProjectServiceRead(t *testing.T) {
	...
	t.Run("GetProjectsByUser success", func(t *testing.T) {
		...
		mockProject.EXPECT().ListProjectsByUserID("1").Return(projects, nil)
