...
func registerProjectRoutes(r *gin.RouterGroup, h *handlers.Handlers, authMw *middleware.AuthMiddleware) {
	projects := r.Group("/projects")
	{
		...
		projects.PUT("/:id", authMw.GroupManager(middleware.FromProjectIDInPayload()), h.Project.UpdateProject)
		projects.DELETE("/:id", authMw.GroupManager(middleware.FromProjectIDInPayload()), h.Project.DeleteProject)
