...
func (h *GroupStorageHandler) GetMyGroupStorages(c *gin.Context) {
	...
	permByGroup := make(map[string]map[string]storage.GroupStoragePermission)
	pvcIDsByGroup := make(map[string]map[string]struct{})
	for _, p := range userPerms {
		if permByGroup[p.GroupID] == nil {
			permByGroup[p.GroupID] = make(map[string]storage.GroupStoragePermission)
		}
		permByGroup[p.GroupID][p.PVCID] = p
		if pvcIDsByGroup[p.GroupID] == nil {
			pvcIDsByGroup[p.GroupID] = make(map[string]struct{})
		}
		pvcIDsByGroup[p.GroupID][p.PVCID] = struct{}{}
	}

	var result []storage.GroupPVCWithPermissions
	for gid, pvcIDs := range pvcIDsByGroup {
		pvcs, err := h.storageMgr.ListGroupPVCs(c.Request.Context(), gid)
		if err != nil {
			continue
		}

		pvcByID := make(map[string]storage.GroupPVCSpec, len(pvcs))
		pvcByName := make(map[string]storage.GroupPVCSpec, len(pvcs))
		for _, pv := range pvcs {
			pvcByID[pv.ID] = pv
			if pv.PVCName != "" {
				pvcByName[pv.PVCName] = pv
			}
		}

		for pvcID := range pvcIDs {
			pv, ok := pvcByID[pvcID]
			if !ok {
				pv, ok = pvcByName[pvcID]
			}
			if !ok {
				continue
			}

			up := permByGroup[gid][pvcID]
