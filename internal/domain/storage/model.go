...
type GroupPVC struct {
	...
	GroupID      string    `gorm:"not null;index;size:21"`                 // Foreign key to group
...
func (m *StorageHub) BeforeCreate(tx *gorm.DB) (err error) {
	if m.ID == "" {
...
type PersistentVolumeClaim struct {
	...
	ProjectID        *string   `gorm:"column:project_id;size:21"`
...
func (m *PersistentVolumeClaim) BeforeCreate(tx *gorm.DB) (err error) {
	if m.ID == "" {
		m.ID, err = gonanoid.New()
	}
	return
